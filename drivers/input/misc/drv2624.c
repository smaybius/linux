...
static int drv2624_reg_write(struct drv2624_data *pDRV2624, unsigned char reg,
			     unsigned char val)
{
	int nResult;
...
static int drv2624_bulk_read(struct drv2624_data *pDRV2624, unsigned char reg,
...
static int drv2624_bulk_write(struct drv2624_data *pDRV2624, unsigned char reg,
...
static int drv2624_set_bits(struct drv2624_data *pDRV2624, unsigned char reg,
 ...
 **/
	...
	if (nResult < 0) {
		dev_err(pDRV2624->dev, "%s: change mode not Done nResult= %d\n",
			__func__, nResult);
		goto end;
	}

	nResult = drv2624_set_bits(pDRV2624, AUTO_CAL_TIME_REG,
				   AUTO_CAL_TIME_MASK,
				   AUTO_CAL_TIME_AUTO_TRIGGER);
	if (nResult < 0) {
		dev_err(pDRV2624->dev, "%s: set bits not Done nResult = %d\n",
			__func__, nResult);
		goto end;
	}
...
static int drv2624_get_calibration_result(struct drv2624_data *pDRV2624)
{
	...
	cal_gain = drv2624_reg_read(pDRV2624, DRV2624_REG_LOOP_CONTROL) &
		   BEMFGAIN_MASK;
	if (cal_gain < 0)
		goto end;
...
/*show calibrtion*/
static ssize_t drv2624_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	int nResult = 0;
			     ...
			     const char *buf, size_t size)
{
	...
	if ((buf[0] == 'C') && (buf[1] == 'A') && (buf[2] == 'L') &&
	    (buf[3] == 'I') && (buf[4] == 'B')) {
		nResult = dev_auto_calibrate(tdev);
	} else if ((buf[0] == 'D') && (buf[1] == 'I') && (buf[2] == 'A') &&
		   (buf[3] == 'G')) {
				...
				struct drv2624_waveform_sequencer *pSequencer)
{
	...
	for (i = 0; i < DRV2624_SEQUENCER_SIZE; i++) {
		...
		if (pSequencer->msWaveform[i].mnEffect != 0) {
			if (i < 4)
				loop[0] |= (pSequencer->msWaveform[i].mnLoop
					    << (2 * i));
			else
				loop[1] |= (pSequencer->msWaveform[i].mnLoop
					    << (2 * (i - 4)));
	...
	if (len == 1)
		...
		nResult = drv2624_bulk_write(pDRV2624, DRV2624_REG_SEQUENCER_1,
					     effects, len);
				     ...
				     unsigned char effect)
{
	...
	playback_interval =
		(pDRV2624->msWaveformSetting.mnInterval == INTERVAL_5MS) ? 5 :
									   1;
	...
	if (fw != NULL) {
		...
		if (size > 1024) {
			...
			if ((pDRV2624->fw_header.fw_magic != DRV2624_MAGIC) ||
				...
				for (i = 0; i < pDRV2624->fw_header.fw_effCount;
				     i++)
					pDRV2624->mnEffectTimems[i] =
						drv2624_get_effect_timems(
							pDRV2624, i);
			}
		}
	} else
				 ...
				 loff_t *offset)
{
	...
	case HAPTIC_CMDID_REG_READ:
		if (length == 1) {
			...
			if (p_kBuf != NULL) {
				nResult = drv2624_bulk_read(
					pDRV2624, pDRV2624->mnCurrentReg,
					p_kBuf, length);
				if (nResult >= 0) {
					nResult = copy_to_user(buff, p_kBuf,
							       length);
					if (nResult != 0) {
						/* Failed to copy all the data, exit */
						dev_err(pDRV2624->dev,
				  ...
				  size_t len, loff_t *off)
{
	...
	case HAPTIC_CMDID_REG_WRITE:
		if ((len - 1) == 2)
			nResult = drv2624_reg_write(pDRV2624, p_kBuf[1],
						    p_kBuf[2]);
		else if ((len - 1) > 2)
			nResult = drv2624_bulk_write(pDRV2624, p_kBuf[1],
						     &p_kBuf[2], len - 2);
		else
					 ...
					 struct ff_effect *old)
{
	...
	case FF_CONSTANT:
		...
		dev_info(pDRV2624->dev, "%s: length(%d), level(%d)\n", __func__,
			 play->length, play->rtp_input);
		break;
	case FF_PERIODIC:
#ifdef NEED_RELOAD_FIRMWARE
		if (pDRV2624->fw_header.fw_magic == 0) {
			nResult = request_firmware_nowait(
				THIS_MODULE, FW_ACTION_UEVENT, "drv2624.bin",
				pDRV2624->dev, GFP_KERNEL, pDRV2624,
				drv2624_firmware_load);
			dev_info(pDRV2624->dev,
		...
		time_ms =
			pDRV2624->mnEffectTimems[data[CUSTOM_DATA_EFFECT_IDX] -
						 1];
...
static void haptics_playback_work_routine(struct work_struct *work)
{
	...
	case FF_CONSTANT:
		...
		nResult = drv2624_reg_write(pDRV2624, DRV2624_REG_RTP_INPUT,
					    0x7f);
		if (nResult < 0)
			break;
				    ...
				    int val)
{
	...
	dev_dbg(pDRV2624->dev, "%s: mnEffectType(%d) WorkMode(%d)\n", __func__,
		pDRV2624->mnEffectType, pDRV2624->mnWorkMode);
...
static void drv2624_haptics_set_gain(struct input_dev *dev, u16 gain)
{
	...
	play->rtp_input = (gain - LIGHT_MAGNITUDE) * 255 /
			  (STRONG_MAGNITUDE - LIGHT_MAGNITUDE);
	//play->rtp_input = gain;
...
static void drv2624_init(struct drv2624_data *pDRV2624)
{
	...
	wavesetting.mnLoop = drv2624_reg_read(pDRV2624, DRV2624_REG_MAIN_LOOP) &
			     0x07;
...
static int drv2624_i2c_probe(struct i2c_client *client)
{
	...
	pDRV2624 = devm_kzalloc(&client->dev, sizeof(struct drv2624_data),
				GFP_KERNEL);
	if (pDRV2624 == NULL) {
	...
	if (gpio_is_valid(pDRV2624->msPlatData.mnGpioINT)) {
		nResult = gpio_request(pDRV2624->msPlatData.mnGpioINT,
				       "DRV2624-IRQ");
		if (nResult < 0) {
		...
		nResult = request_threaded_irq(
			pDRV2624->mnIRQ, drv2624_irq_handler, NULL,
	...
	request_firmware_nowait(THIS_MODULE, FW_ACTION_UEVENT, "drv2624.bin",
				&(client->dev), GFP_KERNEL, pDRV2624,
				drv2624_firmware_load);
